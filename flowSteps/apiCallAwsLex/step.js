 ...
 */
step.apiCallAwsLex = function (inputs) {
...
function setApiUri(options) {
	var API_URL = config.get("AWS_LEX_API_BASE_URL");
...
function setRequestHeaders(options) {
	...
	sys.logs.debug('[awsLex] Set header Bearer');
	...
	if (headers.Accept === undefined || headers.Accept === null || headers.Accept === "") {
		sys.logs.debug('[awsLex] Set header accept');
...
function getAccessTokenForAccount(account) {
	...
	sys.logs.info('[awsLex] Getting access token for account: '+account);
	var installationJson = sys.storage.get('installationInfo-AWS Lex---'+account) || {id: null};
	var token = installationJson.token || null;
	var expiration = installationJson.expiration || 0;
	if (!token || expiration < new Date().getTime()) {
		sys.logs.info('[awsLex] Access token is expired or not found. Getting new token');
		...
		sys.logs.info('[awsLex] Saving new token for account: ' + account);
		sys.storage.replace('installationInfo-AWS Lex---'+account, installationJson);
	}
	return token;
}
